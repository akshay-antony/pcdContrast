...
def mesh_to_pcd(mesh, num_points=1000):
    ...
    if mesh.visual.kind == 'vertex' and hasattr(mesh.visual, 'vertex_colors'):
        ...
        face_colors = np.mean(vertex_colors[mesh.faces], axis=1)  # Average color per face
...
def render_pcd_with_intrinsics(pcd, image_resolution=(512, 512), intrinsic_matrix=None):
    ...
    points_homo = np.hstack([points_normalized, np.ones((points.shape[0], 1))])  # Add 1 for homogeneous coordinates
...
def process_file(filename,
                 ...
                 viewpoints=[(0, 90, 0), (0, 45, 0), (0, 45, 120), (0, 45, 240),
    ...
    filename_formatted = filename.replace("model_normalized.obj", "models/model_normalized.obj")
    ...
    for roll, pitch, yaw in viewpoints:
        rotated_pcd = apply_rotation_to_pcd_trimesh(pcd, roll=roll, pitch=pitch, yaw=yaw)
